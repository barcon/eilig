...
__kernel void EllpackTraceOffset(const EILIG_SIZE_T rows, const EILIG_SIZE_T cols, const EILIG_SIZE_T width, __global EILIG_SIZE_T* count, __global EILIG_SIZE_T* position, __global EILIG_SCALAR* data, __global EILIG_SCALAR* partial, const EILIG_SIZE_T offset)
{
	size_t i = get_global_id(0);
	size_t n;
	...
	if (i < rows)
	{
		...
		for (size_t k = 0; k < n; k++)
		{
			if (position[i * width + k] == i + offset)
			{
				partial[i] = data[i * width + k];
				break;
			}
...
__kernel void EllpackAddSl(const EILIG_SIZE_T rows, const EILIG_SIZE_T cols, const EILIG_SIZE_T width, __global EILIG_SIZE_T* count, __global EILIG_SIZE_T* position, __global EILIG_SCALAR* data, __global EILIG_SIZE_T* countRes, __global EILIG_SIZE_T* positionRes, __global EILIG_SCALAR* dataRes)
{
	size_t i = get_global_id(0);
	size_t j = get_global_id(1);
	size_t n;
...
__kernel void EllpackDiagonalOffset(const EILIG_SIZE_T rows, const EILIG_SIZE_T cols, const EILIG_SIZE_T width, __global EILIG_SIZE_T* count, __global EILIG_SIZE_T* position, __global EILIG_SCALAR* data, const EILIG_SIZE_T widthRes, __global EILIG_SIZE_T* countRes, __global EILIG_SIZE_T* positionRes, __global EILIG_SCALAR* dataRes, const EILIG_SIZE_T offset)
{
	size_t i = get_global_id(0);
	size_t n;
	...
	if (i < rows)
	{
		...
		for (size_t j = 0; j < n; j++)
		{
			...
			{
				countRes[i] = 1;
				positionRes[i * widthRes + 0] = i + offset;
				dataRes[i * widthRes + 0] = data[i * width + j];
				break;
			}
